...
const skinMaterial = new THREE.MeshStandardMaterial({
	color: 0x4b2c20, // Dark Brown for shaft and testicles
	roughness: 0.8,
	metalness: 0.0,
	side: THREE.DoubleSide,
});
const glansMaterial = new THREE.MeshStandardMaterial({
	color: 0xa45a2a, // Orangutan Rusty Brown
	roughness: 0.3,
	metalness: 0.05,
});
...
function updateArbre(scale, glansControls = null) {
	...
	const glansMaterialCustom = new THREE.MeshStandardMaterial({
		color: glansControls ? glansControls.color : 0xa45a2a,
...
function updateGlansShape(glansControls) {
	...
	glans.material.color.setHex(glansControls.color || 0xa45a2a);
	...
	genital.traverse((child) => {
		if (child.isMesh && child.material) {
			...
			if (child.material === skinMaterial || child.name !== "penis_tip") {
				child.material.color.setHex(0x4b2c20);
				child.material.needsUpdate = true;
			}
			// Update glans to brown
			if (child.name === "penis_tip") {
				child.material.color.setHex(0xa45a2a);
				child.material.needsUpdate = true;
			}
		}
	});
