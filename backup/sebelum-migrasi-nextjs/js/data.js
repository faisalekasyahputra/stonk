...
async function findPairAddress(contractAddress, chainId) {
	...
	try {
		...
		if (!genericResponse.ok) {
			throw new Error(`Search API HTTP error! Status: ${genericResponse.status}`);
		}
		...
		console.warn("[PAIR_FINDER] No suitable pair found from token/search APIs.");
		return null;
	} catch (error) {
		console.error(
...
function updateDisplayWithData(tokenData, updateArbreCallback) {
	...
	document.getElementById("ca-box-address").textContent = TOKEN_ADDRESS;

	// 2. Token Info Panel (Top Right)
	document.getElementById("token-name").textContent = tokenData.baseToken.name;
	...
	statusContainer.querySelector(".status-dot").className =
...
function updateSizeFromMarketCap(valueForScaling, updateArbreCallback) {
	...
	if (changeEl) {
		let changeColor = "#86efac";
		if (Math.abs(displayPercent) > 20)
			changeColor = displayPercent > 0 ? "#00ff00" : "#ff0000";
		else if (Math.abs(displayPercent) > 10)
			changeColor = displayPercent > 0 ? "#88ff88" : "#ff8888";
		else if (Math.abs(displayPercent) > 5)
			changeColor = displayPercent > 0 ? "#bbffbb" : "#ffbbbb";
