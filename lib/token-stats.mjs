...
export async function loadStats(mint, providers = { stonk, dex, birdeye, pump, launchlab, helius }) {
	...
	const [onchain, ...values] = await Promise.all([read("launchlab"), read("stonk"), read("dex")]);
	...
	if (!(stats.priceUsd > 0) || (stats.marketCapUsd == null && stats.fdvUsd == null)) {
		if (!(onchain?.priceUsd > 0)) values.push(await read("pump"));
		stats = mergeStats(mint, values);
	}
	...
	if (onchain?.priceUsd > 0 && onchain.fdvUsd > 0) {
		stats.priceUsd = onchain.priceUsd;
		stats.fdvUsd = onchain.fdvUsd;
		// A delayed market cap would take display priority over the live total-supply valuation.
		stats.marketCapUsd = null;
		delete stats.fieldSources.marketCapUsd;
		stats.fieldSources.priceUsd = onchain.source;
		stats.fieldSources.fdvUsd = onchain.source;
		stats.status = "ready";
	}
