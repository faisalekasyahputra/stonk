...
const APE = "C1MHyoTJpRTeS9AQCyspNVu2EWAYCZwmJ1jNkEArFP1f";
...
export function mergeStats(address, providers) {
	const result = { address, name: null, fieldSources: {}, checkedAt: new Date().toISOString() };
...
async function accounts(addresses) {
	const body = await json(process.env.SOLANA_RPC_URL || "https://api.mainnet-beta.solana.com", {
		method: "POST",
...
export async function launchlab(mint) {
	const quotes = [...new Set([process.env.STATS_QUOTE_MINT || APE, SOL])].filter(validMint);
...
export async function loadStats(mint, providers = { stonk, dex, birdeye, pump, launchlab }) {
	const attempts = [];
