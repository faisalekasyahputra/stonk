...
const SKY = 0x1235ad; // cobalt blue, matches the preloader board
...
const SKY_CFG = {
	...
	lines: [
		...
		{ color: "#dff3ff", width: 3, y: 0.58, vol: 0.006 },
		{ color: "#4de0ff", width: 3, y: 0.68, vol: 0.005 },
	],
...
function gridLayer() {
	...
	const draw = (k) => {
		...
		if (k.boxed) {
			g.fillStyle = `rgba(40,85,190,${srnd(0.5, 0.8)})`;
		...
		if (k.pct) {
			...
			g.fillStyle = k.boxed ? "#b4ddf0" : `rgba(150,210,235,${srnd(0.6, 0.95)})`;
...
function buildSky() {
	...
	const base = new THREE.Mesh(
		shell(424),
		new THREE.MeshBasicMaterial({ color: SKY, side: THREE.BackSide, depthWrite: false, fog: false }),
	);
	base.renderOrder = -3;
	group.add(base);
...
function floorTexture() {
	...
	grad.addColorStop(0, 'rgba(60,120,230,0.18)');
	grad.addColorStop(0.5, 'rgba(0,0,0,0)');
	grad.addColorStop(1, 'rgba(60,120,230,0.12)');
	g.fillStyle = grad;
	g.fillRect(0, 0, size, size);
	// grid seam
	g.strokeStyle = 'rgba(100,190,235,0.55)';
	...
	g.strokeStyle = 'rgba(100,190,235,0.2)';
	g.lineWidth = 1;
...
function buildFloor() {
	...
	return new THREE.Mesh(
		geo,
		new THREE.MeshBasicMaterial({ map, color: map ? 0xffffff : 0x0a236b }),
	);
}
